...
const buildAlternativeImageUrls = (imagePath, recordId = null) => {
  ...
  // Primary: Direct PocketBase URLs (correct format based on your API)
  if (recordId) {
    // Direct PocketBase URLs using the correct format you provided
    ...
    // Netlify proxy URLs (for HTTPS deployment)
    alternatives.push(
        ...
        const transformedHistory =
          result.data.history?.map((item) => {
            ...
            if (item.wajah) {
              ...
              // Test the correct format URL manually
              const correctFormatUrl = `http://52.77.219.198:8090/api/files/pbc_2982428850/${item.id}/${item.wajah}`;
              console.log("🎯 Correct format URL:", correctFormatUrl);
