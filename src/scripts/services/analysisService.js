...
class AnalysisService {
  ...
  buildImageUrl(imagePath, recordId = null) {
    ...
    // Use the correct PocketBase API format
    // Format: http://52.77.219.198:8090/api/files/pbc_2982428850/{record_id}/{filename}
    if (recordId) {
      // Direct PocketBase URL (correct format based on your API)
      const directUrl = `http://52.77.219.198:8090/api/files/pbc_2982428850/${recordId}/${filename}`;
      console.log("🔗 Built direct PocketBase URL:", directUrl);

      // For production deployment, use Netlify proxy
      const proxyUrl = `/files/pbc_2982428850/${recordId}/${filename}`;
      ...
      const finalUrl = isDevelopment ? directUrl : proxyUrl;
      console.log(
        "🎯 Final URL selected:",
        finalUrl,
        "(development:",
